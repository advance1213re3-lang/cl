...
function CAL2_pushCasesToApp_(sh, cols) {
  ...
  const readCols = Math.max(cols.caseNo, cols.vendor, cols.propertyName, cols.room,
                            ...
                            cols.address||0, cols.keyInfo||0, cols.comment||0, cols.orderUrl||0);
  ...
  for (let i = 0; i < vals.length; i++) {
    ...
    rows.push({
      ...
      address:  cols.address ? String(row[cols.address - 1] || '') : '',
      key_info: cols.keyInfo ? String(row[cols.keyInfo - 1] || '') : '',
      note:     cols.comment ? String(row[cols.comment - 1] || '') : '',
      order_url: cols.orderUrl ? String(row[cols.orderUrl - 1] || '') : ''
    });
  }
  ...
  if (!force && ev && rev && rev.workDateCol) {
    try {
      ...
      if (evStart.getTime() !== sheetStart.getTime() && (locked || (rec && rec.sig === sig))) {
        ...
        // ★カレンダーで動いた日付を cl_cases(ダッシュボード) にも反映（毎分quietでもここは更新する）
        try {
          const __cfg = CAL2_CFG.supabase;
          if (__cfg && __cfg.url && __cfg.anonKey) {
            const __url = __cfg.url.replace(/\/+$/, '') + '/rest/v1/cl_cases?case_no=eq.' + encodeURIComponent(caseNo) + '&kbn=eq.' + CAL2_mode_().calendarKey;
            UrlFetchApp.fetch(__url, { method: 'patch', contentType: 'application/json', headers: { apikey: __cfg.anonKey, Prefer: 'return=minimal' }, payload: JSON.stringify({ work_date: Utilities.formatDate(evStart, CAL2_CFG.tz, 'yyyy-MM-dd') }), muteHttpExceptions: true });
          }
        } catch (__e) { CAL2_log_('ERROR', caseNo, '', '', 'cl_cases日付逆同期: ' + (__e && __e.message ? __e.message : __e)); }
